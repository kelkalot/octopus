"""Build the three-panel headline Figure 1 for the paper.

Panel A — coefficient axis: non-monotonic disclaimer rate on #26221
under a coefficient sweep on identity probes, plus monotonic
anchor (#22082) and breakdown anchor (#2932) on cluster rate.
...
vs coefficient for single-feature #29108, joint
{#29108, #26221, #4405}, and random direction (mean over K=5).
Cosine-to-baseline numbers at the matched coefficients are reported
in Table 4.

Output: figures/figure1_three_axes.pdf and .png
"""
...
def regex_degenerate(text: str) -> bool:
    import re
    t = text.strip()
...
def panel_a_coefficient_axis(ax):
    """Top-context label vs causal axis: U-shape on #26221."""
    # Build per-coef rates from data files
    def rates_from_file(path, cluster_words=None, disc=False):
        ...
        for r in d:
            ...
            if disc:
                # AI-disclaimer regex (subset)
                hit = bool(re.search(
                    r"\bas an ai\b|\bas a large language model\b|"
                    r"\bi am an ai\b|\bi'?m an ai\b|"
                    r"\bin the same way humans do\b|"
                    r"\bi (?:don'?t|do not) (?:have|experience|possess)",
                    t))
            else:
                # cluster lemma hit
                cluster = ("consciousness reality experience meaning existence "
                           "philosophy understanding emotion mind".split())
                hit = any(w in t for w in cluster)
            by_coef[r["coefficient"]].append(hit)
        return {c: mean(v) for c, v in by_coef.items()}

    f2 = rates_from_file(DATA / "f2_identity_26221.json", disc=True)
    coefs = sorted(f2)
    disc_rates = [f2[c] for c in coefs]

    # Add #22082 monotonic and #2932 breakdown controls (cluster hit rate
    # on the same intervention prompts as #29108 dose-response)
    f22082 = rates_from_file(DATA / "feat22082_dose.json", disc=False)
    f2932  = rates_from_file(DATA / "feat2932_dose.json", disc=False)
    ...
    ax.plot(c22, [f22082[c] * 100 for c in c22],
            ...
            label="#22082 cluster rate (monotonic control)")
    ax.plot(c29, [f2932[c] * 100 for c in c29],
            "^:", color=C_NEG, lw=1.2, ms=4, alpha=0.85,
            label="#2932 cluster rate (breakdown control)")
...
def panel_b_joint_condition(ax):
    ...
    def cluster_rate(records, prompt, coef):
        ...
        for r in bucket:
            t = r["completion"].lower()
            if any(w in t for w in cluster):
                hits += 1
        return hits / len(bucket)

    # Single #29108 at coef -500 (from feat29108_dose.json)
...
def panel_c_norm_probe(ax):
    ...
    Cosine-to-baseline numbers at the matched coefficients are
    reported in Table 4 of findings.tex."""
    # From the existing JSON outputs of norm_probe.py and the random control.
    # Single #29108
    ...
    # Random: load and aggregate over directions per coef using
    # the per-prompt geometry stored on every record
    ...
    for r in rd:
        ...
        if key in seen_dir_coef_prompt: continue
        seen_dir_coef_prompt.add(key)
        by_coef_norms[r["coefficient"]].append(r["norm_ratio_at_prompt_end"])
    ...
    # Annotate the matched-geometry crossing
    # at single -1000: 1.57; joint -500: 1.51; random -1000: 1.56
